...
describe('UserService', () => {
    ...
    describe('createUser', () => {
        it('should throw ConflictError if email already exists', async () => {
            mockUserRepository.findByEmailOrPhone.mockResolvedValue({ id: '1', email: 'test@test.com' });
            await expect(userService.createUser({ email: 'test@test.com', fullName: 'Test', role: Role.USER })).rejects.toThrow(ConflictError);
        });
    ...
    describe('updateProfile', () => {
        it('should throw NotFoundError if user does not exist', async () => {
            mockUserRepository.findById.mockResolvedValue(null);
            await expect(userService.updateProfile('1', { fullName: 'New Name' })).rejects.toThrow(NotFoundError);
        });
    });
});
