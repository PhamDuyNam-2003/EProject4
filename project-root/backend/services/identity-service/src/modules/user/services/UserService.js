...
export class UserService {
    ...
    async approveIdentityVerification(userId) {
        ...
        const anyUser = user;
        if (anyUser.role === Role.AGENT ||
            (anyUser.agentProfile &&
                anyUser.agentProfile.approvalStatus === UserStatus.ACTIVE)) {
            logger.warn(`Duyệt KYC thất bại: Tài khoản ID ${userId} đã được phê duyệt làm Agent từ trước`);
    ...
    async rejectIdentityVerification(userId, reason) {
        ...
        if (anyUser.role === Role.AGENT ||
            (anyUser.agentProfile &&
                anyUser.agentProfile.approvalStatus === UserStatus.ACTIVE)) {
            logger.warn(`Từ chối KYC thất bại: Tài khoản ID ${userId} đã là Đối tác chính thức.`);
