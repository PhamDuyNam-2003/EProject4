...
export const getAllHotels = async (
  ...
  const data = await (prisma as any).hotel.findMany({
    where: Object.keys(where).length > 0 ? where : undefined,
    skip,
    take: limit,
  });
  
  const total = await (prisma as any).hotel.count({
    where: Object.keys(where).length > 0 ? where : undefined,
  });
...
export const getRoomTypesByHotelId = async (hotelId: string) => {
  return await (prisma as any).roomType.findMany({ where: { hotelId } });
};

export const getReviewsByHotelId = async (hotelId: string) => {
...
export const lockRoom = async (roomTypeId: string, quantity: number, checkIn: Date, checkOut: Date) => {
  // Simplistic lock logic: check if there's enough inventory
  const roomType = await (prisma as any).roomType.findUnique({ where: { id: roomTypeId } });
